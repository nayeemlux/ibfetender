...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-md-12 text-center">
            <h5 style="font-weight: bold;padding-top: -25px;">Results of</h5>{{@$allData['0']['exam_type']['name']}}
        </div>
        <div class="col-md-12">
<hr style="border: solid 1px; width: 100%; color: #DDD; margin-bottom: 0px">

            <table border="0" width="100%" cellspacing="1" cellpadding="1" class="text-center">
                <tbody>
                <tr>
                    <td><strong>Sessiion:</strong>{{@$allData['0']['year']['name']}}</td>
                    <td></td>
                    <td></td>
...
@php
$allMarks=App\Model\StudentMarks::where('year_id',$data->year_id)->where('class_id',$data->class_id)->
   where('exam_type_id',$data->exam_type_id)->where('student_id',$data->student_id)->get();
$total_marks=0;
$total_point=0;
foreach ($allMarks as $value){
$count_fail=App\Model\StudentMarks::where('year_id',$value->year_id)->where('class_id',$value->class_id)->
...
<td>{{$key+1}}</td>
                    ...
                    @php
$total_subject=App\Model\StudentMarks::where('year_id',$data->year_id)->where('class_id',$data->class_id)->
   where('exam_type_id',$data->exam_type_id)->where('student_id',$data->student_id)->get()->count();
    $total_grade=0;
  $point_for_latter_grade=(float)$total_point/(float)$total_subject;
